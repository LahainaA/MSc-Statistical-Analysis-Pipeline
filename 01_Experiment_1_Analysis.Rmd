...
# Initialize list to store tables for Appendix
appendix_list <- list()
...
```{r load, echo=FALSE}
# This pipeline is identical for Exp 1 (Horizontal), Exp 2 (Upward), and Exp 3 (Downward). It was run individually, each in a different rmd file. The below represents Exp 1.
...
# Define Groups using LINEAR Baseline data 
...
# Calculate the "Interaction Score" per participant
...
# CHECK FOR BALANCE
# complete 2x2 data for every participant
...
# RUN MIXED ANOVA (Switched to aov_ez to capture MSE)
...
ggplot(df_ancova_valid, aes(x = shift_linear_invis_odd, y = mean_error, color = pathVisible, fill = pathVisible)) +
  ...
  # Reference lines (Zero error / Zero shift)
  ...
  # Facet by Geometry to see the effect in Linear vs Sinusoidal
...
#  F.3: Simple Effects of Geometry 
...
cat("*(Insert derivative-based landmark analysis figures or tables here.)*\n")
```
